...
function numberGenerator(num1, num2) {
  // return Math.min(num1, num2) + Math.round(Math.random()) bad solution only incriments min number randomly
  return Math.min + Math.round(Math.max(num1, num2) - Math.min(num1, num2) * Math.round(Math.random()))
}